<?php
/**
 * @package Woordle/Core
 */

const WOORDLE_ROOT_PATH = WP_PLUGIN_DIR . '/woordle';
const WOORDLE_CORE_PATH = WOORDLE_ROOT_PATH . '/core';
const WOORDLE_VENDORS_PATH = WOORDLE_ROOT_PATH . '/vendors';
const WOORDLE_BACKEND_PATH = WOORDLE_ROOT_PATH . '/backend';
const WOORDLE_FRONTEND_PATH = WOORDLE_ROOT_PATH . '/frontend';
const WOORDLE_MODELS_PATH = WOORDLE_BACKEND_PATH . '/post_types';
const WOORDLE_TAXONOMIES_PATH = WOORDLE_BACKEND_PATH . '/taxonomies';

const WOORDLE_BACKEND_RESOURCES_URL = WP_PLUGIN_URL . '/woordle/backend/resourses';
const WOORDLE_FRONTEND_RESOURCES_URL = WP_PLUGIN_URL . '/woordle/frontend/resourses';